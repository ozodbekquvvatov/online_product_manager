...
export const Dashboard: React.FC = () => {
  ...
  const formatSaleData = (sale: any): FormattedSale => {
    try {
      const orderNumber = sale.orderNumber || sale.sale_number || `SOTUV-${sale.id}`;
      ...
      return {
        id: sale.id || 0,
        orderNumber: `SOTUV-${sale.id || 'NOMA\'LUM'}`,
        totalAmount: 0,
        netAmount: 0,
        subtotal: 0,
        taxAmount: 0,
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      setError('Ma\'lumotlarni yuklashda xatolik: ' + (error.message || 'Noma\'lum xatolik'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateAllMetrics = (employeesData: Employee[], productsData: Product[], salesData: FormattedSale[]) => {
    try {
      ...
      setError('Metrikalarni hisoblashda xatolik');
    }
  };
  ...
  const getFinancialBreakdown = (): FinancialData[] => {
    const data: FinancialData[] = [
      { name: 'Jami Daromad', value: metrics.totalRevenue, color: '#3b82f6' },
      { name: 'Bugungi Daromad', value: metrics.todayRevenue, color: '#8b5cf6' },
      { name: 'Sof Foyda', value: metrics.totalProfit, color: '#10b981' },
      { name: 'Mahsulotlar Qiymati', value: metrics.totalProductsValue, color: '#f59e0b' },
      { name: 'Ish Haqi Xarajatlari', value: metrics.totalSalaryExpense, color: '#ef4444' },
      { name: 'Mahsulot Xarajatlari', value: metrics.totalProductCost, color: '#dc2626' },
    ...
    return (
      <Container fluid className="px-2 px-sm-3 py-2">
        <div className="text-center py-4">
          ...
          <p className="mt-2 text-muted">Biznes ma'lumotlari yuklanmoqda...</p>
        </div>
      </Container>
    );
  }
  ...
  return (
    <Container fluid className="px-2 px-sm-3 py-2">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div>
          <h4 className="fw-bold mb-0">Biznes Boshqaruv Paneli</h4>
          <p className="text-muted small mb-0">
            {lastUpdated && `Oxirgi yangilanish: ${lastUpdated.toLocaleTimeString('uz-UZ')}`}
          </p>
        </div>
            ...
            <Card.Body className="p-2">
              <div className="d-flex justify-content-between align-items-center">
                <div className="flex-grow-1">
                  <small className="text-muted fw-semibold">Mahsulotlar Qiymati</small>
                  <h6 className="mb-0 fw-bold text-info">{formatCurrency(metrics.totalProductsValue)}</h6>
                  <small className="text-muted">{metrics.totalProducts} ta mahsulot</small>
                </div>
        ...
        <Col xs={12} sm={6} xl={3}>
          <Card className="border-0 shadow-sm h-100 compact-card">
            <Card.Body className="p-2">
              <div className="d-flex justify-content-between align-items-center">
                <div className="flex-grow-1">
                  <small className="text-muted fw-semibold">Sof Foyda</small>
                  <h6 className={`mb-0 fw-bold ${metrics.totalProfit >= 0 ? 'text-success' : 'text-danger'}`}>
                    {formatCurrency(metrics.totalProfit)}
                  </h6>
                  <small className="text-muted">{metrics.profitMargin.toFixed(1)}% foyda marjasi</small>
                </div>
        ...
        <Col xs={12} sm={6} xl={3}>
          <Card className="border-0 shadow-sm h-100 compact-card">
            <Card.Body className="p-2">
              <div className="d-flex justify-content-between align-items-center">
                <div className="flex-grow-1">
                  <small className="text-muted fw-semibold">Jami Daromad</small>
                  <h6 className="mb-0 fw-bold text-primary">{formatCurrency(metrics.totalRevenue)}</h6>
                  <small className="text-muted">{metrics.totalSales} ta sotuv</small>
                </div>
        ...
        <Col xs={12} sm={6} xl={3}>
          <Card className="border-0 shadow-sm h-100 compact-card">
            <Card.Body className="p-2">
              <div className="d-flex justify-content-between align-items-center">
                <div className="flex-grow-1">
                  <small className="text-muted fw-semibold">Bugungi Daromad</small>
                  <h6 className="mb-0 fw-bold text-purple">{formatCurrency(metrics.todayRevenue)}</h6>
                  <small className="text-muted">{metrics.todaySales} ta sotuv</small>
                </div>
        ...
        <Col xs={4} sm={2}>
          <Card className="border-0 shadow-sm h-100 stats-card">
            <Card.Body className="p-1 text-center">
              ...
              <small className="text-muted">Mahsulot</small>
            </Card.Body>
          </Card>
        </Col>
        ...
        <Col xs={4} sm={2}>
          <Card className="border-0 shadow-sm h-100 stats-card">
            <Card.Body className="p-1 text-center">
              ...
              <small className="text-muted">Sotuv</small>
            </Card.Body>
          </Card>
        </Col>
        ...
        <Col xs={4} sm={2}>
          <Card className={`border-0 shadow-sm h-100 stats-card ${metrics.lowStockItems > 0 ? 'warning-card' : ''}`}>
            <Card.Body className="p-1 text-center">
              ...
              <small className="text-muted">Ogohlantirish</small>
            </Card.Body>
          </Card>
        </Col>
        ...
        <Col xs={4} sm={2}>
          <Card className="border-0 shadow-sm h-100 stats-card">
            <Card.Body className="p-1 text-center">
              ...
              <small className="text-muted">Ish Haqi</small>
            </Card.Body>
          </Card>
        </Col>
      ...
      <Card className="border-0 shadow-sm mb-2">
        <Card.Header className="bg-white border-0 py-2 d-flex justify-content-between align-items-center">
          <h6 className="mb-0 fw-semibold small d-flex align-items-center">
            ...
            Kunlik Sotuvlar (7 kun)
          </h6>
          <Badge bg="light" text="dark" className="small">
            {dailySales.reduce((sum, day) => sum + day.orders, 0)} ta buyurtma
          </Badge>
        </Card.Header>
          ...
          <div className="table-responsive">
            <Table hover className="mb-0 small table-github">
              <thead className="bg-light">
                <tr>
                  <th className="py-2 px-3 border-0">Sana</th>
                  <th className="py-2 px-3 border-0">Kun</th>
                  <th className="py-2 px-3 border-0 text-end">Sotuvlar</th>
                  <th className="py-2 px-3 border-0 text-end">Foyda</th>
                  <th className="py-2 px-3 border-0 text-end">Foyda %</th>
                  <th className="py-2 px-3 border-0 text-center">Buyurtmalar</th>
      ...
      <Row className="g-2 mb-2">
        <Col xs={12} xl={8}>
          <Card className="border-0 shadow-sm h-100">
            <Card.Header className="bg-white border-0 py-1">
              <h6 className="mb-0 fw-semibold small">Moliyaviy Tahlil</h6>
            </Card.Header>
            <Card.Body className="p-0">
        ...
        <Col xs={12} xl={4}>
          <Card className="border-0 shadow-sm h-100">
            <Card.Header className="bg-white border-0 py-1">
              <h6 className="mb-0 fw-semibold small">Sotuv Trendi (7 kun)</h6>
            </Card.Header>
            <Card.Body className="p-0">
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={getTrendingData()}>
                  ...
                  <YAxis 
                    fontSize={9} 
                    tickFormatter={(value) => {
                      if (value === 0) return '0 UZS';
                      if (value < 1000) return `${value} UZS`;
                      return `${(value / 1000).toFixed(0)}k UZS`;
                    }} 
                  />
                  <Tooltip 
                    formatter={(value) => [formatCurrency(Number(value)), 'Sotuv']}
                  />
                  <Line 
                    type="monotone" 
                    dataKey="value" 
