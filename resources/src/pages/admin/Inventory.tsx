...
export const Inventory: React.FC = () => {
    ...
    try {
      ...
      if (productsResponse.data.success) {
        const productsData = productsResponse.data.data.map((product: any) => ({
          ...
          category: 'Umumiy',
          supplier: 'Standart Ta`minotchi',
          last_restocked: product.updated_at ? new Date(product.updated_at).toLocaleDateString() : 'Hech qachon'
        }));
      ...
      console.error('Inventar ma\'lumotlarini yuklashda xatolik:', error);
      setError('Inventar ma\'lumotlarini yuklash muvaffaqiyatsiz tugadi');
    } finally {
      setLoading(false);
    }
  };
    ...
    const mockMovements: StockMovement[] = [
      {
        id: 1,
        product_id: 1,
        product_name: productsData[0]?.name || 'Mahsulot 1',
        type: 'in',
        quantity: 50,
        reason: 'restock',
      ...
      },
      {
        id: 2,
        product_id: 2,
        product_name: productsData[1]?.name || 'Mahsulot 2',
        type: 'out',
        quantity: 10,
        reason: 'sale',
        date: new Date().toISOString().split('T')[0],
    ...
    return (
      <div className="d-flex justify-content-between align-items-center mt-3 px-3 py-2 border-top">
        <small className="text-muted">
          Ko'rsatilmoqda: {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredProducts.length)} dan {filteredProducts.length}
        </small>
  ...
  const handleRestock = async () => {
    if (!selectedProduct || !restockQuantity || parseInt(restockQuantity) <= 0) {
      setError('Iltimos, to\'g\'ri miqdorni kiriting');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        setSuccess(`${selectedProduct.name} mahsulotiga ${restockQuantity} dona muvaffaqiyatli qo'shildi`);
        setRestockQuantity('');
      ...
      console.error('Mahsulotni qayta to\'ldirishda xatolik:', error);
      setError('Mahsulotni qayta to\'ldirish muvaffaqiyatsiz tugadi');
    }
  };
  ...
  return (
    <Container fluid className="fade-in px-3 px-sm-4 py-3">
      ...
      <Row className="mb-4">
        <Col>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
              <h4 className="fw-bold mb-1">Inventar</h4>
              <p className="text-muted mb-0">Mahsulot zaxiralaringizni kuzatish</p>
            </div>
      ...
      <Row className="mb-4 g-3">
        <Col xs={12} sm={6} lg={3}>
          <MetricCard
            ...
            subtitle={`${categories.length} kategoriya`}
        ...
        <Col xs={12} sm={6} lg={3}>
          <MetricCard
            ...
            value={formatCurrencyUZS(totalInventoryValue)}
            subtitle="Jami zaxira qiymati"
        ...
        <Col xs={12} sm={6} lg={3}>
          <Card className="dashboard-card h-100">
            <Card.Body className="p-3">
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  <p className="text-muted mb-1 small">Kam Zaxira</p>
                  <h6 className="mb-1 fw-bold">{lowStockProducts.length}</h6>
                  <small className="text-muted">Qayta buyurtma kerak</small>
                </div>
        ...
        <Col xs={12} sm={6} lg={3}>
          <Card className="dashboard-card h-100">
            <Card.Body className="p-3">
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  <p className="text-muted mb-1 small">Zaxira Tugagan</p>
                  <h6 className="mb-1 fw-bold">{outOfStockProducts.length}</h6>
                  <small className="text-muted">Mavjud emas</small>
                </div>
      ...
      <Card className="mb-3">
        <Card.Body className="py-3">
          <Row className="g-2 align-items-end">
            <Col md={4}>
              <Form.Label className="small fw-semibold">Qidirish</Form.Label>
            ...
            <Col md={3}>
              ...
              >
                ...
                <option value="in_stock">Zaxirada</option>
                <option value="low_stock">Zaxira Kam</option>
                <option value="out_of_stock">Zaxira Tugagan</option>
              </Form.Select>
            </Col>
            <Col md={2}>
        ...
        <Card className="mb-3 border-warning">
          <Card.Header className="bg-warning bg-opacity-10 py-2">
            <div className="d-flex align-items-center">
              ...
              <h6 className="mb-0 fw-semibold text-warning">Kam Zaxira Ogohlantirishlari</h6>
      ...
      <Card>
        <Card.Header className="bg-white py-3">
          <div className="d-flex justify-content-between align-items-center">
            <h6 className="mb-0 fw-semibold">Mahsulotlar Ro'yxati ({filteredProducts.length})</h6>
                    ...
                    <div className="flex-grow-1">
                      ...
                      <div className="mb-2">
                        <small className="text-muted d-block">
                          SKU: {product.sku || 'Mavjud emas'}
                        </small>
                      </div>

                      <div className="row g-1 mb-2">
                        <div className="col-6">
                          <small className="text-muted d-block">Zaxira</small>
                          <strong className="small">{product.current_stock} {product.unit_of_measure}</strong>
                        </div>
                        <div className="col-6">
                          <small className="text-muted d-block">Narx</small>
                          <strong className="small text-success">{formatCurrencyUZS(product.selling_price)}</strong>
                        </div>
                      </div>

                      <div className="d-flex justify-content-between align-items-center">
                        <small className="text-muted">
                          Qiymat: {formatCurrencyUZS(product.cost_price * product.current_stock)}
                        </small>
                ...
                <Table hover className="mb-0" style={{ minWidth: '1000px' }}>
                  <thead className="bg-light position-sticky top-0">
                    <tr>
                      ...
                      <th style={{ width: '10%' }}>Kategoriya</th>
                      <th style={{ width: '8%' }}>Zaxira</th>
                      <th style={{ width: '8%' }}>Min. Zaxira</th>
                      <th style={{ width: '10%' }}>Holati</th>
                      <th style={{ width: '10%' }}>Sotish Narxi</th>
                      <th style={{ width: '12%' }}>Zaxira Qiymati</th>
                      <th style={{ width: '7%' }} className="text-center">Harakatlar</th>
                    </tr>
                  </thead>
                  <tbody>
                    {currentProducts.map((product) => (
                      <tr key={product.id} className="align-middle">
                        ...
                        <td className="text-success fw-semibold">{formatCurrencyUZS(product.selling_price)}</td>
                        <td className="fw-semibold">{formatCurrencyUZS(product.cost_price * product.current_stock)}</td>
                        <td>
      ...
      <Modal show={showProductModal} onHide={() => setShowProductModal(false)} centered scrollable size="lg">
        ...
        <Modal.Body className="py-3">
          {selectedProduct && (
            <Row className="g-3">
              <Col md={6}>
                <h6 className="text-muted border-bottom pb-2 small">Mahsulot Ma'lumotlari</h6>
                <div className="mb-2">
                  <small className="text-muted d-block">Nomi</small>
                  <strong>{selectedProduct.name}</strong>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">SKU</small>
                  <code className="small">{selectedProduct.sku || 'Mavjud emas'}</code>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">Kategoriya</small>
                  <strong className="small">{selectedProduct.category}</strong>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">Ta'minotchi</small>
                  <strong className="small">{selectedProduct.supplier}</strong>
                </div>
              </Col>
              <Col md={6}>
                <h6 className="text-muted border-bottom pb-2 small">Zaxira Ma'lumotlari</h6>
                <div className="mb-2">
                  <small className="text-muted d-block">Joriy Zaxira</small>
                  <strong className="small">{selectedProduct.current_stock} {selectedProduct.unit_of_measure}</strong>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">Minimal Zaxira</small>
                  <strong className="small">{selectedProduct.min_stock} {selectedProduct.unit_of_measure}</strong>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">Tannarx</small>
                  <strong className="small">{formatCurrencyUZS(selectedProduct.cost_price)}</strong>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">Sotish Narxi</small>
                  <strong className="small text-success">{formatCurrencyUZS(selectedProduct.selling_price)}</strong>
                </div>
                <div className="mb-2">
                  <small className="text-muted d-block">Foyda Marjı</small>
                ...
                <div>
                  <small className="text-muted d-block">Zaxira Holati</small>
      ...
      <Modal show={showRestockModal} onHide={() => setShowRestockModal(false)} centered>
        <Modal.Header closeButton className="py-3">
          <Modal.Title className="h6">Qayta To'ldirish</Modal.Title>
        </Modal.Header>
        <Modal.Body className="py-3">
          {selectedProduct && (
            <>
              <div className="mb-3">
                <small className="text-muted d-block">Mahsulot</small>
              ...
              <Form.Group>
                <Form.Label className="small fw-semibold">Qo'shish Uchun Miqdor *</Form.Label>
                <Form.Control
                  type="number"
              ...
              {restockQuantity && (
                <div className="mt-3 p-2 bg-light rounded">
                  <small className="text-muted d-block">Yangi Zaxira Darajasi:</small>
